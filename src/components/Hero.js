...
const Hero = (props) => { 
  ...
  if (props.hero_img.length === 0) {
    ...
    if(props.ctaTxt) {ctaTxt = props.ctaTxt;} else {ctaTxt = `This is set in 'Call to Action' on WP Page Editor.`;}
    return (
      <div id="heroImg" className="clean-block clean-hero" style={{ backgroundImage: 'url(' + props.hero_img + ')' }}>
        ...
        <div className="clean-hero-text">
          ...
          <Link to={props.ctaBtn} className="react-link" ><button className="btn btn-outline-light btn-lg" type="button">{ctaTxt}</button></Link>
        </div>
      </div>
    )
  }
